            ...
            while attempts > 0:
                ...
                if choice.lower() == 'y':
                    if random.randint(1, 2) == 1:
                        ...
                        self.minions.append({"name": monster.name, "rank": rank, "power": monster.power})
...
def play_game():
    ...
    while not player.is_defeated():
        command = input("Enter 'g' to find a gate, 'm' to view minions, or 'q' to quit: ")
        if command == 'q':
            break
        elif command == 'm':
            player.show_minions()
        ...
        for monster in monsters:
            ...
            while not monster.is_defeated() and not player.is_defeated():
                ...
                if action == 'a':
                    ...
                    ability = input(f"Choose ability: {list(ABILITIES.keys())}: ")
